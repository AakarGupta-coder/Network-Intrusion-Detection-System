...
class SQLInjectionParser:
    ...
    def parse(self):
        ...
        patterns = [
            "' or '1'='1' --",
            "' or 1=1 --",
            "' or '1'='1", # Without comment, still an injection attempt
            "' or 1=1"     # Without comment
        ]
...
class NIDSApp:
    def __init__(self, master):
        self.master = master
        master.title("NIDS Dashboard - Aakar Gupta")
    ...
    def _create_widgets(self):
        ...
        ttk.Label(header_frame, text="Grammar-Based Intrusion Detection System", style="Header.TLabel").pack(side=tk.LEFT, padx=(0, 20))
        ...
        self.manual_frame = ttk.LabelFrame(self.left_panel, text="Payload Analysis", style="Dark.TLabelframe", padding="10")
        ...
        self.tree = ttk.Treeview(self.history_frame, columns=("Time", "Payload", "Type"), show="headings", height=8)
        self.tree.heading("Time", text="Time", anchor=tk.W)
        self.tree.heading("Payload", text="Payload", anchor=tk.W)
        self.tree.heading("Type", text="Type", anchor=tk.W)
        self.tree.column("Time", width=140, stretch=tk.NO)
        self.tree.column("Payload", width=220, stretch=tk.YES)
        self.tree.column("Type", width=120, stretch=tk.NO)
    ...
    def _update_dashboard(self):
        ...
        for entry in reversed(self.intrusion_log):
            self.tree.insert("", tk.END, values=(entry['time'], entry['payload'], ', '.join(entry['types'])))
    ...
    def _process_payload(self, payload):
        ...
        if detected_types:
            self._log_message(f"[{now}] 🚨 INTRUSION DETECTED: '{payload}'", "red")
            self.intrusion_log.append({"time": now, "payload": payload, "types": detected_types})
            for d_type in detected_types:
                self.detection_counts[d_type] += 1
        else:
            self._log_message(f"[{now}] ✅ Benign Payload: '{payload}'", "green")
    ...
    def _update_graphs(self):
        ...
        self.pie_ax.set_title("Benign vs. Malicious", color=mpl_text)
    ...
    def _show_about_info(self):
        ...
        about_info = """
This Network Intrusion Detection System (NIDS) is a practical application of concepts from the Theory of Computation, specifically Formal Languages and Automata Theory.

Core Logic: The system uses a set of parsers built upon Context-Free Grammars (CFGs). Each grammar precisely defines the structure of a known cyber threat (e.g., SQL Injection). When a data payload is analyzed, the system attempts to parse it against these grammars. A successful parse indicates that the payload's structure matches a threat signature, and an alert is generated. This method ensures high-precision detection with a low rate of false positives for the defined threats.
...
"""
    ...
    def _show_more_info(self):
        ...
        ttk.Label(info_frame, text=f"Detected Payload: '{self.selected_payload}'", font=('Segoe UI', 10, 'italic'), style='TLabel').pack(anchor=tk.W, pady=(0,5))
        ...
        info_content = f"""
Grammar: {intrusion_data['grammar']}
...
"""
    ...
    def _update_network_monitor(self):
        if not self.network_monitoring_active:
            return
